...
class RIM(PLCore):
	def __init__(self, d_hidden, n_units, n_active_RIMs, n_layers=1, rnn="LSTM", bidirectional=False, **kwargs):
		super().__init__(**kwargs)
		self.rnn = rnn
		# self.d_features and self.d_seq_out from parent class
		if args.TRAINER_CONFIG["accelerator"]=="cpu":
			device = torch.device("cpu")
		else:  # gpu or auto works here
			device=torch.device("cuda:0")
		self.visualization_device = device  # override "cpu" if necessary
		self.example_input_array = None  # TODO training might stop when this is not None
		...
		self.fc = nn.Linear(d_hidden * n_units * (bidirectional + 1), self.d_features * self.d_seq_out)
	...
	def _shared_step(self, x, y):
		...
		if self.rnn == 'LSTM':
			_, h, _ = self.RIM(x)
		else:  # TODO GRU not working properly?
			_, h, = self.RIM(x)

		y_pred = self.fc(h).view(b_size, -1, self.d_features)
